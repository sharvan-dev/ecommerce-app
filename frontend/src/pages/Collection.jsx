...
const Collection = () => {
  ...
  return (
    <div className='flex flex-col sm:flex-row gap-1 sm:gap-10 pt-10 border-t'>
     ...
     {/* Filter Options */}
     <div className='min-w-660'>
      <p onClick={()=>setShowFilter(!showFilter)} className='my-2 text-xl flex items-center cursor-pointer gap-2'>FILTERS
        <img className={`h-3sm:hidden ${showFilter ? 'rotate-90' : ''}`} src={assets.dropdown_icon} alt="" />
      </p>
      {/* Category Filter */}
      <div className={`border border-gray-300 pl-5 py-33 mt-6 ${showFilter ? '' :'hidden'} sm:block`}>
        ...
        <div className='flex flex-col gap-2 text-sm font-light text-gray-700'>
           ...
           <p className='flex gap-2'>
            <input className='w-3' type="checkbox" value={'Footwear'} checked={category.includes('Footwear')} onChange={toggleCategory} /> Footwear
           </p>
        </div>
      </div>
      {/* SubCategory Filter */}
      <div className={`border border-gray-300 pl-5 py-33 my-5 ${showFilter ? '' :'hidden'} sm:block`}>
        <p className='mb-6 text-sm font-medium'>TYPE</p>
        <div className='flex flex-col gap-2 text-sm font-light text-gray-700'>
           <p className='flex gap-2'>
      ...
      <div className='flex-1'> 
         ...
         <div className='flex justify-between text-base sm:text-2xl mb-4'>
                ...
                {/* Porduct Sort */}
                <select onChange={(e)=>setSortType(e.target.value)} className='bordetr-2 border-gray-300 text-sm px-2'>
          ...
          <div className='grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 gap-y-6'>
            {
              filterProducts.map((item,index)=>(
                <ProducItem key={index} name={item.name} id={item._id} price={item.price} image={item.image} />
              ))
            }
          </div>
